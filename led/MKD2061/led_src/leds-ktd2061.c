...
static void ktd2058_select_color(struct ktd2058_chip *chip, int color, u8 sel)
{
	...
	if (color == 0)				isel = 0x88; //color0
	else if (color == 1)		isel = 0xFF; //color1		
	else if (color == 3)		isel = 0x00; //OFF
	else if (color == 4)		isel = sel;
...
static void ktd2058_breath_white(struct ktd2058_chip *chip)
{
...
static ssize_t ktd2058_racing_demo_store(struct device *dev, 
												struct device_attribute *attr, const char *buf, size_t size)
{
	struct led_classdev *led_cdev = dev_get_drvdata(dev);
	struct ktd2058_chip *chip = container_of(led_cdev, struct ktd2058_chip, cdev_flash);
