...
static char *usage[] = {
	...
	"         vndr_cmd_dump", "         auth_assoc_timeout_cfg"};
...
{
	char prm_name[MAX_CONFIG_VARIABLE_LEN];
	sscanf(config_line, "%31[^=]=%s", prm_name, val);
}
...
static int process_get_txpwrlimit(int argc, char *argv[])
{
	...
	if (argc < 4) {
		printf("Error: invalid no of arguments\n");
#if defined(SDAW693) || defined(PCIEAW693)
		printf("mlanutl mlanX/uapX get_txpwrlimit [0/0x80/0x10/0x11/0x12/0x13/0x1f/0x20/0x21/0x22/0x23/0x24/0x25/0x26/0x27/0x2f/0xff]\n");
#else
