...
u32 awd8833c_set_motor_speed(u8 speed)
{		
	//if ((speed == 0) || (speed == 0)
	if (speed == 0)
	{
	...
	speed--;
    //g_full_step_delay = 1800 * (u32)speed; 	
	g_full_step_delay = (u32)18000 -  (u32)1800 * (u32)speed; 	
...
/**
  ...
  *            @arg AW_HALF_STEP: 1.
  *            @arg AW_FULL_STEP: 0.
  *        u32step  Motor drive pluse count
...
void awd8833c_step_function(u8 direction, u8 mode_select, u32 u32step)
{
	...
	if (direction == AW_FORWARD) 
	{
		...
		{
			for (aw_count = 0; aw_count < u32step; aw_count++) 
			{
...
u32 awd8833c_degree2step(u8 Motor, u32 degree)
{
	...
	if(Motor == AW_PAN_MOTOR)
		step = (degree*100/75+5)/10*85; 		//PAN motor: Half-Step; Step Angle : 7.5 degree 
	else
		step = (degree*10/15+5)/10; 			//Rising motor: Full-Step; Step Angle : 15 degree 
	
	printk("Degree: %u\tStep: %u\n",degree,step);
	return step;
}
...
void awd8833c_timer_start(u32 u32msecs)
{	 
    mod_timer(&timer, jiffies + msecs_to_jiffies(u32msecs));
	//clear the Flag
	m_isTimeout=0;
}

void awd8833c_motor_launch_by_time(u8 motor, u8 direction, u8 speed, u32 u32msecs)
{	
	/* init gpio state */
	awd8833c_gpio_init();
	awd8833c_set_enable(AW_TRUE, motor);
	awd8833c_set_motor_speed(speed);
	
	awd8833c_timer_start(u32msecs);
	
	if(motor == AW_PAN_MOTOR)
		awd8833c_timer_function(direction, AW_HALF_STEP);
	else	
		awd8833c_timer_function(direction, AW_FULL_STEP);
	
	/* set go sleep */
	awd8833c_set_enable(AW_FALSE, motor);	
}



void awd8833c_motor_launch_by_step(u8 motor, u8 direction, u8 speed, u32 u32step)
{
	/* init gpio state */
	awd8833c_gpio_init();
	awd8833c_set_enable(AW_TRUE, motor);
	...
	if(motor == AW_PAN_MOTOR)
		awd8833c_step_function(direction, AW_HALF_STEP,u32step);	
	else	
		awd8833c_step_function(direction, AW_FULL_STEP,u32step);	
...
void awd8833c_motor_launch_by_degree(u8 motor, u8 direction, u8 speed, u32 u32degree)
{
	...
	awd8833c_set_enable(AW_TRUE, motor);
	awd8833c_set_motor_speed(speed);
	u32step = awd8833c_degree2step(motor, u32degree);
