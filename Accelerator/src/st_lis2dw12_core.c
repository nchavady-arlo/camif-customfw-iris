...
static int st_lis2dw12_fill_accel_channels(struct st_lis2dw12_hw *hw)
{
	...
	hw->st_lis2dw12_acc_channels = devm_kzalloc(hw->dev,
						    hw->acc_num_channels * sizeof(struct iio_chan_spec),
						    GFP_KERNEL);
							
	if (!hw->st_lis2dw12_acc_channels)
		return -ENOMEM;
		      ...
		      enum st_lis2dw12_hw_id hw_id, struct regmap *regmap)
{
	...
	printk("st_lis2dw12 device_register\n");
	/* debug */
//	for (i = 0; i < ST_LIS2DW12_ID_MAX; i++) 
//	{
//		indio_dev = hw->iio_devs[i];
//		channels = indio_dev->channels;
//
//		printk("=== IIO Device %d ===\n", i);
//		for (j = 0; j < indio_dev->num_channels; j++) {
//			printk("Channel %d: type=%d, channel=%d, scan_index=%d\n",
//				j,
//				channels[j].type,
//				channels[j].channel,
//				channels[j].scan_index);
//		}
//
//		printk("Registering IIO device %d\n", i);
//		err = devm_iio_device_register(hw->dev, indio_dev);
//		if (err) {
//			printk("Failed to register IIO device %d, err=%d\n", i, err);
//			return err;
//		}
//	}
