...
{	
	...
	if (m_bDataReceive == true)
	{
		...
		UarRcvLen = UART_ReceiveBuffer(m_u8Buffer, UART_BUFFER_MAX_LEN);
...
void Uart_WaitAndSaveDataToBuffer(int intTimeout_ms)
{
	...
	printf("\n[%s] intTimeout_ms=%d \n", __func__, intTimeout_ms);  
	
	for (int i=0; i<intTimeout_ms/10; i++)
	{
		UarRcvLen = UART_ReceiveHandler();
	...
	if (UarRcvLen > 0)
	{
		printf(" Rcv:%s(%d,%d) \n ", m_u8Buffer, UarRcvLen, m_intCount);
		
		Uart_Buffer_Data_Print(UarRcvLen);
	}
}
//==============================================================================
