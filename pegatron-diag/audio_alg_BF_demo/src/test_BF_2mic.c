...
int main(int argc, char *argv[])
{
	...
	sprintf(outfileName,"%s","/tmp/BFOut.pcm");
    ...
    // fread(input, sizeof(char), 44, fin); // read header 44 bytes (if file type is wav)
	...
	while(fread(input, sizeof(short), bf_init.point_number*bf_init.channel, fin))
	{
		...
		ret = IaaBf_Run(handle,input,output,&direction);			//No use the real DOA = always Forward
