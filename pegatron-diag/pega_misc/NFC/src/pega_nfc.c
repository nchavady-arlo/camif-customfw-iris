...
int Pega_NFC_Device_init(void)
{
    ...
    printf("[pega_misc] Init Semphore!\n");
    ret = sem_init(&rfal_sem, 0, 0);
    if (ret != 0) {
        goto error;
    }
    /* Initialize RFAL and run the demo */
    printf("[pega_misc] Init NFC config!\n");
    demoIni();
    printf("[pega_misc] Done Init\n");

    return 0;
error:
    printf("[pega_misc] Fail to init NFC\n");
    return 1;
}

static void* Pega_NFC_TaskHandler(void* argv)
{			 
	 (void)argv;
	...
	prctl(PR_SET_NAME, THREAD_PROC_11); //set the thread name
	//printf("\n%s\n", __func__);
	printf("[pega_misc] Start NFC Task\n");
...
{              
     //printf("\n%s", __FUNCTION__);	
     printf("[pega_misc] Create NFC Task.\n");    
     pthread_create(&m_pthread_id, NULL, &Pega_NFC_TaskHandler, NULL); 
}
