...
/* PEGA Diag Marco */
#define NFC_DEFAULT_TYPE        "RFAL_NFC_DEFAULT"
#define NFC_DEFAULT_UID         "RFAL_UID_DEFAULT"
#define NFC_DEFAULT_DATA        "RFAL_DATA_DEFAULT"
/*
 ******************************************************************************
 * GLOBAL MACROS
 ******************************************************************************
 ...
 ******************************************************************************
 */
struct NFC_Info NFC_Buff = { 
    .NFC_Type = "RFAL_NFC_DEFAULT",
    .NFC_UID  = "RFAL_UID_DEFAULT",
    .NFC_Data = "RFAL_DATA_DEFAULT"
};

/*! Device type  array  */
const char* Diag_NFCDevTypeStr[] = {
    ...
    "RFAL_NFC_LISTEN_TYPE_PROP",   // 6
    [10] = "RFAL_NFC_POLL_TYPE_NFCA", // 10
    [11] = "RFAL_NFC_POLL_TYPE_NFCB", // 11
    [12] = "RFAL_NFC_POLL_TYPE_NFCF", // 12
    [13] = "RFAL_NFC_POLL_TYPE_NFCV", // 13
    [15] = "RFAL_NFC_POLL_TYPE_AP2P", // 15
    [16] = "RFAL_NFC_DEFAULT"         // 16
};
/*
******************************************************************************
...
void demoHexDump( uint8_t* pBuff, uint8_t nLen)
{
     uint8_t i;
                    ...
                    case RFAL_NFC_LISTEN_TYPE_NFCA:
                        ...
                        switch( nfcDevice->dev.nfca.type )
                        {
                            ...
                            default:
                                ...
                                demoNFC_play_sound();
                                Page_SetNFCBuf(Diag_NFC_Type, Diag_NFCDevTypeStr[RFAL_NFC_LISTEN_TYPE_NFCA]);
                                Page_SetNFCBuf(Diag_NFC_UID, hex2Str( nfcDevice->nfcid, nfcDevice->nfcidLen));
                                demoT2t();
                                break;
                        }
                        break;
                    ...
                    case RFAL_NFC_LISTEN_TYPE_NFCV:
                        {
                            ...
                            Page_SetNFCBuf(Diag_NFC_Type,Diag_NFCDevTypeStr[RFAL_NFC_LISTEN_TYPE_NFCV]);
                            Page_SetNFCBuf(Diag_NFC_UID,hex2Str(devUID, RFAL_NFCV_UID_LEN));
...
static void demoNfcv( rfalNfcvListenDevice *nfcvDev )
{
    ...
    */
    ...
    if(err != ERR_NONE)
    {
        Page_SetNFCBuf(Diag_NFC_Data, "Read Block: FAIL");
    }else{
        Page_SetNFCBuf(Diag_NFC_Data, hex2Str( &rxBuf[1], DEMO_NFCV_BLOCK_LEN));
    }
...
static void demoT2t( void )
{
    ...
    if(err!=ERR_NONE)
    {
        ...
        Page_SetNFCBuf(Diag_NFC_Data, hex2Str( rxBuf, RFAL_T2T_READ_DATA_LEN));
    }
...
{
	 #if 1	 
	 system("/utilities/play_wav /audio/16K_16bit_Stereo_Beep.wav");
	 sleep(1);
	 #endif
}

uint16_t Page_SetNFCBuf(uint8_t attr,void *value)
{
    switch(attr)
    {
        case Diag_NFC_Type:
                strncpy(NFC_Buff.NFC_Type, (const char *)value, MAX_STR_LENGTH - 1);
                NFC_Buff.NFC_Type[MAX_STR_LENGTH - 1] = '\0';
                printf("NFC Type: %s\n",NFC_Buff.NFC_Type);
                break;
    
        case Diag_NFC_UID:        
                strncpy(NFC_Buff.NFC_UID, (char *)value, MAX_STR_LENGTH - 1);
                NFC_Buff.NFC_UID[MAX_STR_LENGTH - 1] = '\0';
                printf("NFC UID: %s\n",NFC_Buff.NFC_UID);
                break;
    
        case Diag_NFC_Data:
                strncpy(NFC_Buff.NFC_Data, (char *)value, MAX_STR_LENGTH - 1);
                NFC_Buff.NFC_Data[MAX_STR_LENGTH - 1] = '\0';
                printf("NFC Data: %s\n",NFC_Buff.NFC_Data);
                break;
        default:
                printf("do nothing!\n");
...
void* Page_GetNFCInfo(uint8_t data_type)
{
    switch(data_type)
    {
        case Diag_NFC_Type:
            return &NFC_Buff.NFC_Type;
            break;

        case Diag_NFC_UID:
            return &NFC_Buff.NFC_UID;
            break;

        case Diag_NFC_Data:
            return &NFC_Buff.NFC_Data;
            break;
...
uint16_t Page_ClearNFCBuf(void)
{
    strncpy(NFC_Buff.NFC_Type, NFC_DEFAULT_TYPE, MAX_STR_LENGTH - 1);
    NFC_Buff.NFC_Type[MAX_STR_LENGTH - 1] = '\0';

    strncpy(NFC_Buff.NFC_UID, NFC_DEFAULT_UID, MAX_STR_LENGTH - 1);
    NFC_Buff.NFC_UID[MAX_STR_LENGTH - 1] = '\0';

    strncpy(NFC_Buff.NFC_Data, NFC_DEFAULT_DATA, MAX_STR_LENGTH - 1);
    NFC_Buff.NFC_Data[MAX_STR_LENGTH - 1] = '\0';
